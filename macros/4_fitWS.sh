...
quickFit -w ${WORKSPACENAME} -f ${WORKSPACEFILE} -d ${DATASETNAME} -p mu_signal=0 \
-o ${OUTPUTNAME} --savefitresult 1 --hesse 1 --minos 1 2>&1 |tee log_MONOTOP_minos.txt
...
quickFit -w ${WORKSPACENAME} -f ${WORKSPACEFILE} -d ${DATASETNAME} -p mu_signal=0 \
-o ${OUTPUTNAME} --savefitresult 1 --hesse 1 --minos 1 2>&1 |tee log_combined_minos.txt
