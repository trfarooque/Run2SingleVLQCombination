...
//check inputs
  ...
  nm_data = (RooDataSet*)w->data(dataName);
  if (!nm_data)
  {
  ...
  obs_nll = createNLL(nm_data);//(RooNLLVar*)pdf->createNLL(*data);
  map_snapshots[obs_nll] = "nominalGlobs";
  map_data_nll[nm_data] = obs_nll;
